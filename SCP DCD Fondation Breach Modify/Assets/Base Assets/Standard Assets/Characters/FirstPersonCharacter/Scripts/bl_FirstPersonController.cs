...
public class bl_FirstPersonController : bl_MonoBehaviour
{
    ...
    [SerializeField]
    private AudioClip[] WatertepSounds;
    [SerializeField]
    private AudioClip[] MetalStepSounds;
    [SerializeField]
    ...
    private void PlayFootStepAudio()
    {
        ...
        if (!isClimbing)
        {
            ...
            switch (_tag)
            {
                case "Water":
                    ...
                    break;
                case "Metal":
                    n = Random.Range(1, MetalStepSounds.Length);
                    m_AudioSource.clip = MetalStepSounds[n];
                    m_AudioSource.PlayOneShot(m_AudioSource.clip);
                    // move picked sound to index 0 so it's not picked next time
                    MetalStepSounds[n] = MetalStepSounds[0];
                    MetalStepSounds[0] = m_AudioSource.clip;
                    break;
                default:
        ...
        {
            int n = Random.Range(1, MetalStepSounds.Length);
            m_AudioSource.clip = MetalStepSounds[n];
            m_AudioSource.PlayOneShot(m_AudioSource.clip);
            // move picked sound to index 0 so it's not picked next time
            MetalStepSounds[n] = MetalStepSounds[0];
    ...
    private void GetInput(out float speed)
    {
        ...
        if (State != PlayerState.Climbing)
        {
            if (m_Input.magnitude > 0 && (m_CharacterController.isGrounded))
            {
                ...
                {
                    Debug.LogWarning("not set up state: " + State.ToString());
                }
            }
