...
#region UGUI
        ...
        if (PhotonNetwork.PhotonServerSettings.AppID != string.Empty)
        {
            switch (id)
            {
                ...
                case 8:
                    ...
                    break;
                case 9:
                    PhotonNetwork.ConnectToRegion(CloudRegionCode.kr, AppVersion);
                    break;
                case 10:
                    PhotonNetwork.ConnectToRegion(CloudRegionCode.ru, AppVersion);
                    break;
            }
        }
        else
    ...
    public void ChangeAntiStropic(bool plus)
    {
        if (!plus)
        {
            if (m_stropic < bl_GameTexts.AntiStropicOptions.Length)
            {
                m_stropic--;
                if (m_stropic < 0)
                {
                    m_stropic = bl_GameTexts.AntiStropicOptions.Length - 1;
        ...
        else
        {
            ...
            {
                m_stropic++;
                if (m_stropic > (bl_GameTexts.AntiStropicOptions.Length - 1))
                {
                    m_stropic = 0;
                }
            }
        }
        AntiStrpicText.text = bl_GameTexts.AntiStropicOptions[m_stropic];
    }
